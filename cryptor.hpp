...
//
//  origin base64 eocnde/decode source :
//  https://github.com/ReneNyffenegger/cpp-base64
//

#ifndef NamJunHyeon_CRYPTOR_HPP
#define NamJunHyeon_CRYPTOR_HPP
#include <string>
class cryptor {

private:
  static const std::string &get_static_base64() {
    static const std::string m_base64_chars =
        "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/";
    return m_base64_chars;
  }

  static std::string &get_static_key(const std::string &key = "") {
    static std::string m_key = key.empty() ? "default_key" : key;
    return m_key;
  }
...
public:
  ...
  static std::string encrypt(const char *in, const std::string &key = "") {
    return base64_encode(
        do_xor(std::string(in), key.empty() ? get_static_key() : key));
  }

  template <typename buffer_type>
  static buffer_type encrypt(const buffer_type &in,
                             const std::string &key = "") {
    return base64_encode(do_xor(in, key.empty() ? get_static_key() : key));
  }

  static std::string decrypt(const char *in, const std::string &key = "") {
    return do_xor(base64_decode(std::string(in)),
                  key.empty() ? get_static_key() : key);
  }

  template <typename buffer_type>
  static buffer_type decrypt(const buffer_type &in,
                             const std::string &key = "") {
    return do_xor(base64_decode(in), key.empty() ? get_static_key() : key);
  }

  static const std::string &get_key() { return get_static_key(); }

  static void set_key(const std::string &key) {
    auto &m_key = get_static_key(key);
    m_key = key;
  }
...
private:
  ...
  static buffer_type base64_encode_impl(const buffer_type &in) {
    ...
    while (in_len--) {
      ...
      if (i == 3) {
        ...
        for (i = 0; i < 4; ++i) {
          ret.push_back(get_static_base64().at(char_array_4[i]));
        }
    ...
    if (i) {
      ...
      for (j = 0; j < i + 1; ++j) {
        ret.push_back(get_static_base64().at(char_array_4[j]));
      }
  ...
  static void base64_decode_impl(buffer_type &&encoded_data, out_type &out) {
    ...
    while (in_len-- && (encoded_data[in_] != '=') &&
           is_base64(encoded_data[in_])) {
      char_array_4[i++] = encoded_data[in_];
      in_++;
      if (i == 4) {
        for (i = 0; i < 4; ++i) {
          char_array_4[i] =
              static_cast<uint8_t>(get_static_base64().find(char_array_4[i]));
        }
    ...
    if (i) {
      for (j = 0; j < i; ++j) {
        char_array_4[j] =
            static_cast<uint8_t>(get_static_base64().find(char_array_4[j]));
      }
